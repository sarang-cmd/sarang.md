...
export function TutorPanel() {
  ...
  return <section ref={panel} aria-label="Socratic tutor" className={`fixed z-[80] flex flex-col overflow-hidden border border-line bg-canvas text-ink shadow-float ${docked ? 'bottom-0 right-0 top-[72px] w-[min(400px,100vw)] border-r-0 lg:w-[380px]' : 'bottom-2 right-2 h-[min(700px,calc(100dvh-90px))] w-[min(420px,calc(100vw-16px))] rounded-2xl sm:bottom-5 sm:right-5'}`} style={!docked && position && window.innerWidth >= 768 ? { left: position.x, top: position.y, right: 'auto', bottom: 'auto' } : undefined}>
    <div className="flex items-center gap-2 border-b border-line bg-surface px-3 py-3 sm:px-4">
      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-ink text-canvas"><Sparkles size={15} aria-hidden="true" /></span>
      ...
      {!docked && <button type="button" aria-label="Drag tutor window" title="Drag to move" onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={stopDrag} onPointerCancel={stopDrag} className="hidden h-8 w-8 touch-none cursor-grab items-center justify-center text-muted active:cursor-grabbing md:flex"><Grip size={15} aria-hidden="true" /></button>}
      <button type="button" aria-label={docked ? 'Float tutor' : 'Dock tutor on right'} title={docked ? 'Float tutor' : 'Dock tutor on right'} onClick={() => { setDocked(!docked); setPosition(null); }} className="flex h-8 w-8 items-center justify-center rounded-lg text-muted hover:bg-raised hover:text-ink">{docked ? <Maximize2 size={15} /> : <PanelRight size={16} />}</button>
      <button type="button" aria-label="Minimize tutor" title="Minimize" onClick={minimizeTutor} className="flex h-8 w-8 items-center justify-center rounded-lg text-muted hover:bg-raised hover:text-ink"><Minimize2 size={16} /></button>
      <button type="button" aria-label="Close tutor" title="Close tutor" onClick={() => { request.current?.abort(); closeTutor(); }} className="flex h-8 w-8 items-center justify-center rounded-lg text-muted hover:bg-raised hover:text-ink"><X size={17} /></button>
    </div>
    ...
    {view === 'settings' ? <div role="tabpanel" className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5 text-[12px]">
      ...
      <div aria-live="polite" aria-relevant="additions" className="min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-4">
        ...
        {messages.map((message, index) => <div key={index} className={`rounded-xl px-3.5 py-3 text-[12px] leading-relaxed ${message.role === 'user' ? 'ml-6 bg-ink text-canvas' : 'tutor-answer-fade mr-5 border border-line bg-surface'}`}><span className={`mb-1 block font-mono text-[9px] font-semibold uppercase tracking-widest ${message.role === 'user' ? 'text-canvas/60' : 'text-muted'}`}>{message.role === 'user' ? 'You' : canConnect ? 'Tutor' : 'Offline prompt'}</span>{message.role === 'assistant' ? <MarkdownArticle source={message.content} compact /> : <p className="whitespace-pre-wrap">{message.content}</p>}</div>)}
