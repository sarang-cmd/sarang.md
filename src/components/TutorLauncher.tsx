...
export function TutorLauncher() {
  ...
  return <button type="button" aria-label={minimized ? 'Expand tutor' : 'Open Socratic tutor'} data-tip="Five-subject Socratic tutor" onClick={openTutor} className="fixed bottom-5 right-5 z-[80] inline-flex min-h-12 items-center gap-2.5 rounded-full border border-line bg-ink px-4 text-[12px] font-semibold text-canvas shadow-float transition-transform hover:-translate-y-0.5 sm:px-5"><Sparkles size={16} aria-hidden="true" /><span className="hidden sm:inline">{minimized ? 'Continue tutoring' : 'Ask the tutor'}</span></button>;
}
