...
import { ArrowRight, CornerDownLeft, FolderOpen, Menu, Moon, PanelLeft, Search as SearchIcon, Sun, X } from 'lucide-react';
...
export function Header() {
  ...
  const activeTab = pathname === '/search' ? 3 : pathname === '/question-bank' ? 2 : pathname.startsWith('/units') ? 1 : 0;
      ...
      <div className="relative flex h-full w-full items-center justify-between gap-2 px-3 sm:gap-3 sm:px-6 lg:px-8">
        ...
        <nav aria-label="Main navigation" className="absolute left-1/2 hidden -translate-x-1/2 lg:block">
          <div className="relative grid h-10 w-[352px] grid-cols-4 items-center rounded-full border border-line bg-raised p-1">
            <span
              aria-hidden="true"
              className="absolute bottom-1 left-1 top-1 rounded-full bg-ink shadow-sm transition-transform duration-200 ease-out"
