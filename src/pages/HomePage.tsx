import { ArrowRight, ArrowUpRight, BookOpen, FileQuestion, MoveUpRight, NotebookPen, Waves } from 'lucide-react';
...
export function HomePage() {
  ...
  const stats = [
    ...
    { value: String(ready.length).padStart(2, '0'), label: ready.length === 1 ? 'Unit ready to explore' : 'Units ready to explore', detail: mode === 'local' ? 'Loaded from your Markdown' : 'All five AA syllabus strands' },
  ];
  ...
  return (
    <div className="mx-auto w-full max-w-[1320px] px-5 pb-24 pt-14 sm:px-9 sm:pt-20 lg:px-12 xl:px-16 xl:pt-24">
      ...
      <section aria-labelledby="practice-heading" className="mt-6 flex flex-col gap-7 rounded-xl border border-line bg-surface px-7 py-8 sm:flex-row sm:items-center sm:justify-between sm:px-9 sm:py-9">
        <div className="flex items-start gap-5"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-line bg-canvas"><NotebookPen size={20} strokeWidth={1.7} aria-hidden="true" /></span><div><p className="section-label mb-2">02 / Practice space</p><h2 id="practice-heading" className="text-[24px] font-semibold tracking-[-0.045em] sm:text-[27px]">Make it stick.</h2><p className="mt-1 max-w-[510px] text-[13px] leading-relaxed text-muted">Original IB-style Paper 1, 2 and 3 practice across five strands, with independent marking guides to reveal when ready.</p></div></div>
        <Link to="/practice" className="group inline-flex h-11 shrink-0 items-center justify-center gap-2 self-start rounded-full border border-ink px-5 text-[12px] font-semibold transition-colors hover:bg-ink hover:text-canvas sm:self-auto">Open question bank <ArrowRight size={15} className="transition-transform group-hover:translate-x-1" aria-hidden="true" /></Link>
      </section>
      <section aria-label="More ways to study" className="mt-4 grid gap-4 md:grid-cols-2">
        <Link to="/ib-papers" className="group flex items-start gap-4 rounded-xl border border-line p-6 transition-colors hover:border-ink hover:bg-surface"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-surface"><FileQuestion size={19} strokeWidth={1.7} /></span><span className="min-w-0 flex-1"><span className="section-label">03 / Paper practice</span><strong className="mt-2 block text-[18px] tracking-tight">User-supplied transcriptions.</strong><span className="mt-1 block text-[12px] leading-relaxed text-muted">A separate, unverified collection of your supplied Paper 1 and 2 text, plus legacy variants and a numeric checker.</span></span><ArrowUpRight size={16} className="shrink-0 text-muted" /></Link>
        <Link to="/focus" className="group flex items-start gap-4 rounded-xl border border-line p-6 transition-colors hover:border-ink hover:bg-surface"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-surface"><Waves size={19} strokeWidth={1.7} /></span><span className="min-w-0 flex-1"><span className="section-label">04 / Focus studio</span><strong className="mt-2 block text-[18px] tracking-tight">Settle in and begin.</strong><span className="mt-1 block text-[12px] leading-relaxed text-muted">Procedural ambience, a gentle timer and a video queue you can make your own.</span></span><ArrowUpRight size={16} className="shrink-0 text-muted" /></Link>
      </section>
    </div>
  );
}
